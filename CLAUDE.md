...
| 18  | `melhor_envio_service_id` é a escolha do cliente no checkout, não uma garantia. A transportadora pode deixar de atender o trecho entre a compra e a etiqueta — a compra recusada refaz a cotação e o painel oferece quem atende. `shipping_amount` nunca muda na troca: é o que o cliente pagou |
...
| Credenciamento na SEFAZ-MG | 🚫 Bloqueio | **É o que impede a nota hoje.** A SEFAZ responde `Rejeicao: CNPJ [38142237000180] do emitente nao cadastrado na Receita Federal` — a empresa não está credenciada para emitir NF-e em MG. Credenciamento pelo SIARE, **separado por ambiente**: homologação e produção são dois. Já conferido e correto: CNPJ ativo em Muriaé/MG, IE 0042608620043 com dígitos válidos no formato MG, corpo da nota, token e certificado A1 (a resposta veio da SEFAZ, logo a Focus assinou e transmitiu). Nenhuma alteração de código muda isso |
...
| NCM dos produtos | ⚠️ Pendente | 27 dos 28 produtos ativos estão **sem NCM** e a emissão para antes de chamar a Focus. A Bolsa Briana estava com `42022200`, que **não existe** na tabela — a SEFAZ recusou com "Informado NCM inexistente". Sob 4202.22 só existem `42022210` (folhas de plástico) e `42022220` (matérias têxteis); foi posto `42022220` como provisório para destravar o teste. **Classificação fiscal é do contador** — pedir a lista por tipo de peça (bolsa de lona, bolsa de couro, bata, vestido) antes de vender de verdade. NCM errado é multa |
| Domínio canônico e webhooks | ⚠️ Atenção | `NEXT_PUBLIC_APP_URL` é `https://patriciacarreira.com.br` (apex), mas o canônico servido é **www**: o apex responde **308** e redireciona. Webhook cadastrado no apex fica à mercê de o parceiro seguir redirect em POST — muitos não seguem. Medido: apex → 308, `www` → 401 (vivo, recusando requisição sem assinatura), `patricia-carreira-ecommerce.vercel.app` → 401 (ainda vivo, provavelmente é onde os webhooks estão cadastrados e por isso o pagamento continua funcionando). Decidir um canônico, apontar `NEXT_PUBLIC_APP_URL` para ele e recadastrar os três webhooks (MP, ME, Focus) |
