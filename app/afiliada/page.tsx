...
// ── dados da afiliada (virão do Supabase futuramente) ──────────────────────
const afiliada = {
...
export default function AfiliadaPage() {
  ...
  const copyProductLink = (slug: string, id: string) => {
    const link = `${window.location.origin}/produto/${slug}?cupom=${afiliada.coupon}`
                      ...
                      {salesByPeriod[salesPeriod].map((s, i) => (
                        <tr key={i}>
                          ...
                          <td className="num" style={{ textAlign: 'right', color: 'var(--ap-accent)', fontWeight: 500 }}>
                            {fmt(Math.round(s.value * afiliada.commissionPct / 100))}
                          </td>
                          <td>
          ...
          <>
            <div className="card">
              <div className="card-header">
                ...
                <span style={{ fontSize: 11, color: 'var(--ap-text-3)' }}>
                  {fmt(paymentsHistory.reduce((s, p) => s + p.commission, 0))} recebidos no total
                </span>
              </div>
              <div className="card-body flush">
                <div className="table-wrap">
